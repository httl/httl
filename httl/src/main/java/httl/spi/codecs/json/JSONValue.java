...
public class JSONValue {
	...
	public JSONValue(Converter<Object, Map<String, Object>> mc) {
		this.converter = mc;
	}
	...
	public void writeValue(Object obj, JSONWriter jb, boolean writeClass)
		...
		if (encoder != null) {
			...
			Set<String> pns = w.keySet();

			for (String pn : pns) {
				if ((obj instanceof Throwable)
						&& ("localizedMessage".equals(pn) || "cause".equals(pn) || "stackTrace"
								.equals(pn))) {
					continue;
				}
	...
	static {
		...
		e = new Encoder() {
			public void encode(Object obj, JSONWriter jb) throws IOException {
				jb.valueString(new String((byte[]) obj));
			}
		};
		GLOBAL_ENCODER_MAP.put(byte[].class, e);
		...
		e = new Encoder() {
			public void encode(Object obj, JSONWriter jb) throws IOException {
				jb.valueString(new SimpleDateFormat(DATE_FORMAT)
						.format((Date) obj));
			}
		};
